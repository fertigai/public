...
- A target becomes a declared variable, so it must also not differ only by case from one already in `config.dynamic_variables`: an assignment targeting `City` on a branch that declares `city` is a `422`.
...
- An assignment target that collides with a declared variable by case only, or one prefixed `system__` in any casing: both are a `422`, so match the declared spelling exactly.
